...
int main (int argc, const char * argv[]) {
    ...
    string malik_part_file             = folder + config + "_delta_part_m.2dist";
    string malik_out_part_alpha_file   = folder + config + "_alpha_part_m.2dist";
    string malik_out_part_omega_file   = folder + config + "_omega_part_m.2dist";
    string malik_out_square_delta_file = folder + config + "_delta_squares_m.gpl";
    string malik_out_square_alpha_file = folder + config + "_alpha_squares_m.gpl";
    string malik_out_square_omega_file = folder + config + "_omega_squares_m.gpl";
    ...
    /*
    // 3. Improve data partition using Combinatorial
    ...
    */
    ...
    cout << fixed << setprecision(6) << "DELTA arrangement: Malik (tau-Lop) Cost " << endl;
    om->show();
    r_orig->show();
    r_orig->plot(out_square_delta_file);
    r_orig->part2D(out_part_alpha_file);
    ...
    cout << fixed << setprecision(6) << "ALPHA arrangement: Malik (tau-Lop) Cost " << endl;
    cm->show();
    alpham->show();
    alpham->plot(malik_out_square_alpha_file);
    alpham->part2D(malik_out_part_alpha_file);
    
    // 4.c) Malik does not return OMEGA arrangement, so use DELTA
    Arrangement *omegam = (Arrangement *)r_orig;
    cm  = alg_tlop->execute (omegam);
    cout << fixed << setprecision(6) << "OMEGA (DELTA) arrangement:  Malik (tau-Lop) Cost " << endl;
    cm->show();
    omegam->show();
    omegam->plot(malik_out_square_omega_file);
    omegam->part2D(malik_out_part_omega_file);
