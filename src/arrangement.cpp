...
int Arrangement::getColNr (int block) {
    
    int col_nr = 0;
    
    int w = 0;
    for (int i = 0; i < this->getColNr(); i++) {
        w = w + this->getWidth(i);
        if (w > block) {
            col_nr = i;
            break;
...
void  Arrangement::getAbsoluteSpeeds (float *speeds) {
    
    int i = 0;
    for (int col = 0; col < this->col_nr; col++) {
        for (int row = 0; row < getRowNr(col); row++) {
            Rectangle *p = get(row, col);            
            float speed = p->getHeight() * p->getWidth();
            // speeds[p->getRank()] = speed;
            speeds[i++] = speed;
...
void  Arrangement::getRelativeSpeeds (float *speeds) {
    
    this->getAbsoluteSpeeds(speeds);
    float area = this->H * this->W;
    
    for (int i = 0; i < this->P; i++) {
        speeds[i] = speeds[i] / area;
	cout << "i= " << i << " speed: " << speeds[i] << endl;
    }
}



void  Arrangement::getNodes (int *nodes) {

    int i = 0;
    for (int col = 0; col < this->col_nr; col++) {
        for (int row = 0; row < getRowNr(col); row++) {
            Rectangle *p = get(row, col);
            // nodes[p->getRank()] = p->getNode();
	    cout << "i " << i << " node: " << p->getNode() << endl;
            nodes[i++] = p->getNode();
...
void  Arrangement::getHalfPerimeter (float *hper) {
    
    int i = 0;
    for (int col = 0; col < this->col_nr; col++) {
        for (int row = 0; row < getRowNr(col); row++) {
            Rectangle *p = get(row, col);
            // hper[p->getRank()] = p->getHeight() + p->getWidth();
            hper[i++] = p->getHeight() + p->getWidth();
            
        }
    }
}

float  Arrangement::getHalfPerimeter () {
    ...
    for (int col = 0; col < this->col_nr; col++) {
        ...
        for (int row = 0; row < getRowNr(col); row++) {
            ...
            Rectangle *p = get(row, col);
            float perim = p->getHeight() + p->getWidth();
            thper = thper + perim;
...
void Arrangement::show () {
    ...
    for (int col_nr = 0; col_nr < this->col_nr; col_nr++) {
        Column *c = this->getColumn(col_nr);
        cout << " \t " << c->getWidth();
    }
    cout << endl;
#endif
    ...
    for (int col = 0; col < this->getColNr(); col++) {
        ...
        for (int row = 0; row < this->getRowNr(col); row++) {
            Rectangle *p = this->get(row, col);
            cout << p->getRank() << " [" << row << "," << col << "]: \t w: " << p->getWidth() << " \t h: " << p->getHeight()
                 << " \t s: " << p->getHeight() * p->getWidth()            
                 << " \t X: " << p->getCoordX() << " \t Y: " << p->getCoordY() << " siguiente: "<< p->getCoordY() + p->getHeight() << endl;
        }
    }  
    cout << "-------------------------------------------------------------------" << endl << endl;
#endif
...
/*
    for (int m = 0; m < this->M; m++) {
        ...
        for (int p = 0; p < this->P; p++) {
            ...
            string rankintra = itos(ri); ri++; // itos(r->getRankIntra());
