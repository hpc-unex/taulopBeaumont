...
//    int       getSize   ();
    ...
    int       getColNr   (int block);  // Get in what column is a block number
    
    
    void      getAbsoluteSpeeds (float *speeds); // Get the Speed of each proc = area of each rectangle (in blocks)
    void      getRelativeSpeeds (float *speeds); // Get the Speed of each proc = area of each rectangle normalized (1)
    void      getHalfPerimeter  (float *hper);   // Get the Half Perimeter of each process
    void      getNodes          (int   *nodes);  // Get the node of each process/rectangle
    float     getHalfPerimeter  ();              // Get the addition of the half perimeters
